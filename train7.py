    ...
    while True:
        ...
        if ProductName in InpaintingProducts:
            ...
            if ProductName in ExcludedProducts:
                ...
                if ProductName == 't_shirt' or ProductName == 'active_t_shirts':
                    ProductName = 'T-shirt'
                else:
                    ProductName = ProductName.capitalize()
            ...
            if os.path.exists(model_path):
                ...
                os.mkdir(model_path)
            accelerate_command = f'accelerate launch --num_processes 1 train_lora_org.py --instance_data_dir {out_path} --output_dir {model_path} --instance_prompt {ProductName} --resolution=512 --train_batch_size=1 --learning_rate=1e-4   --max_train_steps=2000   --checkpointing_steps 1000'
            ...
            s3_folder_name = f"s3://static.styl.in/models/V1/{str(ProductID)}"
